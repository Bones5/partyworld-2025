...
export default class ProductDetails extends ProductDetailsBase {
  constructor($scope, context, productAttributesData = {}) {
    ...
    this.swatchGroupIdList = $('[id^="swatchGroup"]').map((_, group) =>
      $(group).attr("id"),
    );
    ...
    const placeSwatchLabelImage = (_, label) => {
      ...
      if (
        emptySpaceToScreenRightBorder <
        optionImageWidth + extendedOptionImageOffsetLeft
      ) {
        $optionImage.css(
          "left",
          `${shiftValue > 0 ? -shiftValue : shiftValue}px`,
        );
      }
    };
    ...
    if (context.showSwatchNames) {
      ...
      $.each($productSwatchGroup, (_, element) => {
        ...
        if ($(element).is(":checked"))
          this.showSwatchNameOnOption($(element), $(swatchGroupElement));
      });
    }
  ...
  storeInitMessagesForSwatches() {
    if (
      this.swatchGroupIdList.length &&
      isEmpty(this.swatchInitMessageStorage)
    ) {
      this.swatchGroupIdList.each((_, swatchGroupId) => {
        if (!this.swatchInitMessageStorage[swatchGroupId]) {
          this.swatchInitMessageStorage[swatchGroupId] = $(
            `#${swatchGroupId} ~ .swatch-option-message`,
          )
            .text()
            .trim();
        }
  ...
  setProductVariant() {
    ...
    $.each($("[data-product-attribute]"), (index, value) => {
      ...
      if (type === "date") {
        const isSatisfied = Array.from(value.querySelectorAll("select")).every(
        ...
        if (checked) {
          const getSelectedOptionLabel = () => {
            ...
            const matchLabelForCheckedInput = (inpt) =>
              inpt.dataset.productAttributeValue === checked.value;
  ...
  showSwatchNameOnOption($swatch, $swatchGroup) {
    ...
    const $swatchOptionMessage = $(
      `#${activeSwatchGroupId} ~ .swatch-option-message`,
    );

    $("[data-option-value]", $swatchGroup).text(swatchName);
    $swatchOptionMessage.text(
      `${this.swatchInitMessageStorage[activeSwatchGroupId]} ${swatchName}`,
    );
  ...
  updateDateSelector() {
    this.$scope.each((i, scope) => {
      function updateDays(dateOption) {
        const monthSelector = dateOption.querySelector(
          'select[name$="[month]"]',
        );
      ...
      $(scope).on("change", (e) => {
        const dateOption =
          e.target &&
          e.target.closest &&
          e.target.closest("[data-product-attribute=date]");
      ...
      scope
        .querySelectorAll("[data-product-attribute=date]")
        .forEach((dateOption) => {
          updateDays(dateOption);
        });
    });
