...
export const onUserCarouselChange = ({ data }, context, $slider) => {
  ...
  const $parentContainer = $activeSlider.hasClass("productView-thumbnails")
    ? $activeSlider.parent(".productView-images")
    : $activeSlider;
  ...
  const $carouselContentElement = $(
    "[data-carousel-content-change-message]",
    $parentContainer,
  );
  const carouselContentAnnounceMessage = updateTextWithLiveData(
    context.carouselContentAnnounceMessage,
    activeSlideIdx + 1,
    slidesQuantity,
  );
...
export const onSlickCarouselChange = (e, carouselObj, context) => {
  ...
  const { activeSlideIdx, slidesQuantity } =
  ...
  arrowAriaLabling(
    ...
    slidesQuantity,
    infinite,
    context.carouselArrowAndDotAriaLabel,
  );
...
export default function (context) {
  $("[data-slick]").each((idx, carousel) => {
    ...
    $carousel.on("init breakpoint", (e, carouselObj) =>
      activatePlayPauseButton(e, carouselObj, context),
    );
    $carousel.on("init afterChange", (e, carouselObj) =>
      onSlickCarouselChange(e, carouselObj, context),
    );
    $carousel.on("click", ".slick-arrow, .slick-dots", $carousel, (e) =>
      onUserCarouselChange(e, context),
    );
    $carousel.on("swipe", (e, carouselObj) =>
      onUserCarouselChange(e, context, carouselObj.$slider),
    );
